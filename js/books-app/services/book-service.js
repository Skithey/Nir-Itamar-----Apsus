...
function getNewBooks(bookTitle) {
    return axios.get(`https://www.googleapis.com/books/v1/volumes?q=${bookTitle}:keyes&key=AIzaSyDNV0Izg5wS7rUlbl_VYxjHyR0Drn6Uzho`)
        .then(books => books.data.items.map(book => {
                const bookObj = {
                    id: book.id,
                    title: book.volumeInfo.title,
                    subtitle: book.volumeInfo.subtitle,
                    authors: (!book.volumeInfo.authors) ? 'Mister Lorem Ipsum' : book.volumeInfo.authors,

                    publishedDate: (!book.volumeInfo.publishedDate) ? '1992' : book.volumeInfo.publishedDate,
                    description: 'placerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus interdum ad dictum platea vehicula conubia fermentum habitasse congue suspendisse',
                    pageCount: book.volumeInfo.pageCount,
                    categories: (!book.volumeInfo.categories) ? 'drama' : book.volumeInfo.categories,
                    thumbnail: book.volumeInfo.imageLinks,
                    language: (!book.volumeInfo.language) ? 'en' : book.volumeInfo.language,
                    listPrice: (!book.saleInfo.listPrice) ? 'not for sale' : book.saleInfo.listPrice
                }
                return bookObj
            })
            ...
            // console.log('search word:', bookTitle, ', the book: ', book.searchInfo);
            // console.log('search word:', bookTitle, ', the book: ', book.volumeInfo.title);
...
function addChosenBook(bookToSave) {

    console.log(bookToSave);

    gBooks.unshift(bookToSave)
...
function getBooks() {
    utilsService.saveToStorage('books', gBooks);
